    ...
    node(label) {
        ...
        container('docker') {
                stage('Docker Build & Push Current & Latest Versions') {
                    ...
                    docker build -t dilcher-configuration:${BRANCH_NAME_ESC}-${env.BUILD_NUMBER} --build-arg pypiuser="\${PYPI_USER}" --build-arg pypipass="\${PYPI_PASSWORD}" --build-arg pypidevtag="${BRANCH_NAME_ESC}" -f ./dev.Dockerfile .
